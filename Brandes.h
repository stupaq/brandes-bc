...
namespace brandes {
  ...
  inline GraphGeneric generic_read(const char* file_path) {
  ...
  inline GraphCSR csr_create(const GraphGeneric G) {
    ...
    return GraphCSR { std::move(ptr), std::move(adj) };
  }
  ...
  inline GraphOCSR<Reordering> ocsr_create(const GraphCSR G) {
    ...
    return GraphOCSR<Reordering> { { std::move(bfsno) }, std::move(optr),
      std::move(oadj), std::move(ccs) };
  }

  inline GraphOCSR<Identity> ocsr_pass(GraphCSR G) {
    const VertexId n = G.ptr_.size() - 1;
    std::vector<VertexId> ccs = { 0, n + 1 };
    return GraphOCSR<Identity> { {}, std::move(G.ptr_), std::move(G.adj_),
      std::move(ccs) };
  }
